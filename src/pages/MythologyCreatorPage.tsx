...
export const MythologyCreatorPage: React.FC<MythologyCreatorPageProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const sceneCount = (res.result?.scenes || res.result?.sceneStructure || []).length;
      onShowToast(`Mythology Suite generated with ${sceneCount} scenes! (${res.cost} credits deducted)`, 'success');
    } catch (err: any) {
  ...
  const handleAddScenesToProject = async () => {
    ...
    const scenesToAdd = mythologyResult?.scenes || (mythologyResult as any)?.sceneStructure || [];
    if (!scenesToAdd || scenesToAdd.length === 0) {
    ...
    try {
      for (const scene of scenesToAdd) {
        await api.addScene(activeProject.id, {
          title: `Scene ${scene.sceneNumber}: ${scene.cameraAngle || (scene as any).camera || 'Cinematic Shot'}`,
          duration: scene.duration || 10,
          visualPrompt: scene.visualPrompt || '',
          camera: scene.cameraAngle || (scene as any).camera || 'Cinematic Shot',
          lighting: scene.lighting || 'Divine Golden Hour / Volumetric Astral',
          voiceOverText: scene.hindiVoiceover || (scene as any).hindiNarration || '',
          videoUrl: (scene as any).videoUrl || 'https://assets.mixkit.co/videos/preview/mixkit-nebula-in-deep-space-32008-large.mp4',
          thumbnailUrl: (scene as any).thumbnailUrl || 'https://images.unsplash.com/photo-1579783900882-c0d3dad7b119?w=800&auto=format&fit=crop&q=80',
          status: 'ready'
        });
      }
      onScenesAdded();
      onShowToast(`Added all ${scenesToAdd.length} mythology scenes to "${activeProject.title}"!`, 'success');
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {mythologyResult && (() => {
        const scenes = mythologyResult.scenes || (mythologyResult as any).sceneStructure || [];
        const music = mythologyResult.musicSuggestion || ((mythologyResult as any).musicSuggestions?.join(' • ')) || 'Deep meditative tanpura drone transitioning into thunderous pakhavaj';
        const sfx = mythologyResult.soundEffects || ((mythologyResult as any).sfxSuggestions?.join(' • ')) || 'Cosmic energy hum, sacred bell reverberation';
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl border border-amber-500/40 bg-amber-950/20">
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">
                  {scenes.length} Scenes Generated • Sanskrit / Hindi Sound Cues Ready
                </p>
              </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-zinc-400 font-mono">
                          {sc.cameraAngle || sc.camera || 'Cinematic Shot'}
                        </span>
                      </div>
                      ...
                      <div className="p-3 rounded-lg bg-zinc-900/90 border border-zinc-800">
                        <div className="flex items-center justify-between text-[10px] text-zinc-500 font-semibold mb-1">
                          ...
                          <button
                            onClick={() => copyToClipboard(sc.hindiVoiceover || sc.hindiNarration, 'Hindi Voiceover')}
                        ...
                        <p className="text-xs font-hindi text-amber-100/90 leading-relaxed">
                          {sc.hindiVoiceover || sc.hindiNarration}
                        </p>
                      </div>
